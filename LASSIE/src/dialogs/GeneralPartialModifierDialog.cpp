#include "GeneralPartialModifierDialog.hpp"
...
GeneralPartialModifierDialog::GeneralPartialModifierDialog(
    int modifierType,
    int maxPartialCount,
      ...
      m_maxPartialCount(maxPartialCount)
{
    ...
    connect(m_addNodeButton, &QPushButton::clicked,
            this, &GeneralPartialModifierDialog::addNode);

    connect(m_insertFunctionButton, &QPushButton::clicked,
            this, &GeneralPartialModifierDialog::insertFunction);
...
void GeneralPartialModifierDialog::addNode()
{
    ...
    connect(row.probability, &QLineEdit::textChanged,
        this, &GeneralPartialModifierDialog::updateResultPreview);
...
void GeneralPartialModifierDialog::insertFunction()
{
...
QStringList GeneralPartialModifierDialog::enabledFieldLabels() const
{
    // Field order:
    // Probability, Magnitude, Rate, Width,
    // Detune Spread, Detune Direction, Detune Velocity
    //
    // Modifier type order from Modifiers.ui:
    // 0 TREMOLO
    // 1 VIBRATO
    // 2 GLISSANDO
    // 3 DETUNE
    ...
    if (m_modifierType < 0 || m_modifierType >= 8) {
        return result;
    }
...
void GeneralPartialModifierDialog::trackFocusedEdit(QLineEdit* edit)
{
...
QString GeneralPartialModifierDialog::envelopeOrNA(QLineEdit* edit)
{
    if (!edit) {
        return "";
    }
...
QString GeneralPartialModifierDialog::buildPartialResultString() const
{
...
QStringList GeneralPartialModifierDialog::extractEnvelopeValues(const QString& resultString)
{
    QStringList values;
...
void GeneralPartialModifierDialog::populateFromResultString(const QString& resultString)
{
...
GeneralPartialModifierDialog::EnvelopeEnabled
GeneralPartialModifierDialog::enabledEnvelopeFields() const
{
    ...
    switch (m_modifierType) {
        ...
        case 3: // DETUNE
            // DETUNE should not use PARTIAL mode.
            enabled.probability = false;
            enabled.magnitude = false;
            enabled.width = false;
            enabled.rate = false;
            break;

        case 4: // PHASE_MOD
            // PHASE_MOD should use Diyun's special PartialModifierDialog,
            // not GeneralPartialModifierDialog.
            enabled.probability = false;
            enabled.magnitude = false;
            enabled.width = false;
            enabled.rate = false;
...
void GeneralPartialModifierDialog::applyRowEnabledState(const PartialRow& row) const
{
    const EnvelopeEnabled enabled = enabledEnvelopeFields();
...
void GeneralPartialModifierDialog::removeNode(QWidget* rowContainer)
{
    if (!rowContainer) {
        return;
    }
...
void GeneralPartialModifierDialog::renumberRows()
{
    for (int i = 0; i < m_rows.size(); ++i) {
...
void GeneralPartialModifierDialog::updateResultPreview()
{
    if (!m_resultPreview) {
        return;
    }
...
QString GeneralPartialModifierDialog::getResultString() const
{
