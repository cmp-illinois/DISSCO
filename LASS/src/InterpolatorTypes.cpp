...
Iterator<m_value_type> LinearInterpolator::valueIterator()
{
    ...
    m_time_type maxTime = entries_.back().time_;
    m_value_type mySamplingRate = (getDuration() / maxTime) * getSamplingRate();

    // grab the first value:
    m_value_type lastValue = entries_[0].value_;
    m_time_type lastTime = entries_[0].time_;
    ...
    {
        // get the next entry:
        InterpolatorEntry& lie = entries_[i];
...
Iterator<m_value_type> ExponentialInterpolator::valueIterator()
{
    ...
    {
        // get the next entry:
        InterpolatorEntry& eie = entries_[i];
...
Iterator<m_value_type> CubicSplineInterpolator::valueIterator()
{
    ...
    {
        // get the next entry:
        InterpolatorEntry& csie = entries_[i];
